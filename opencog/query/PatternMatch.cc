 ...
 */
bool BindLink::imply(PatternMatchCallback& pmc, AtomSpace* atomspace, bool check_conn)
{
   if (check_conn and 0 == _virtual.size() and 1 < _components.size())
	...
	bool bogus = remove_constants(_varlist.varset, _pat.clauses, _pat.constants,
	                              _components, _pat.mandatory, _pat.cnf_clauses,
	                              atomspace);
	if (bogus)
	{
